...
- F1 uncovered refusal paths — §5.1 no manifest, §5.2 non-JSON, §5.2 non-object top level have no tests (test-review S5.1) → already scheduled as Test 9 and Test 11 in Phase 1: the parse and §5.1 refusals land in `__tests__/manifest.test.ts`, the top-level-object rule in `__tests__/top-level-fields.test.ts`
